...
class TetrisGrabber:
        ...
        while cap.isOpened():
            ...
            if ret == True:
                ...
                if indeces_first_obj:
                    max_block_y =  np.array(max(indeces_first_obj, key=operator.itemgetter(1)))
                    min_block_y =  np.array(min(indeces_first_obj, key=operator.itemgetter(1)))
    ...
    def get_grid(self, img):
        ...
        if grid[1][9] == 1 and grid[2][9] == 1 and grid[3][9] == 1 and grid[4][9] == 1 and  grid[5][9] == 0:
            grid[2,9] = 0
            grid[1,9] = 0
            grid[3,9] = 0
            grid[4,9] = 0
    ...
    def get_move(self, indeces_prev, indeces):
        ...
        if  np.array_equiv(upper, [0,-1]) and np.array_equiv(lower, [0,-1]):
            return 'DOWN'
        if  (np.array_equiv(upper, [-1,0]) and np.array_equiv(lower, [-1,0]))  or(np.array_equiv(upper, [-1,-1]) and np.array_equiv(lower, [-1,-1])):
            return 'RIGHT'
        if  (np.array_equiv(upper, [1,0]) and np.array_equiv(lower, [1,0]))  or(np.array_equiv(upper, [1,-1]) and np.array_equiv(lower, [1,-1])):
            return 'LEFT'
